...
function keepLargestMaskComponent(alpha, width, height) {
  const active = new Uint8Array(alpha.length);
  for (let i = 0; i < alpha.length; i += 1) {
    if (alpha[i] > MASK_ALPHA_THRESHOLD) active[i] = 1;
  }
  ...
  if (!largestComponent.length) return alpha;

  const filtered = new Uint8Array(alpha.length);
  for (let i = 0; i < largestComponent.length; i += 1) {
    const idx = largestComponent[i];
    filtered[idx] = alpha[idx];
  }
  return filtered;
}
...
function buildMaskDataForRegion(img, width, height) {
  ...
  const filteredAlpha = keepLargestMaskComponent(alpha, width, height);
  const edges = [];
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      const idx = (y * width) + x;
      if (filteredAlpha[idx] <= MASK_ALPHA_THRESHOLD) continue;
      const isEdge =
        x === 0 || y === 0 || x === width - 1 || y === height - 1 ||
        filteredAlpha[idx - 1] <= MASK_ALPHA_THRESHOLD ||
        filteredAlpha[idx + 1] <= MASK_ALPHA_THRESHOLD ||
        filteredAlpha[idx - width] <= MASK_ALPHA_THRESHOLD ||
        filteredAlpha[idx + width] <= MASK_ALPHA_THRESHOLD;
      if (isEdge) edges.push(idx);
    }
  }

  return { image: img, alpha: filteredAlpha, edges };
}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          Ram Selvarajah has been building a member driven mandate since November 2025. Click the regions below to see who's been in the room, or{" "}
          <a
            className="gpiLink"
            href="https://www.ram4change.ca/member-driven#insights"
            target="_blank"
            rel="noreferrer"
          >
            scroll
          </a>{" "}
          to learn about member insights.
        </p>
      </div>
